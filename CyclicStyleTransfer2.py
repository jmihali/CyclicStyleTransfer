...
def run_cyclic_style_transfer(content_image_name=content_image_name, style_image_name=style_image_name, content_layers=content_layers, content_weights=content_weights, style_layers=style_layers,
    ...
    if 'lpsis' in similarity_type_weight.keys():
    ...
    while n_iter[0] <= 2*max_iter:
        ...
        def closure_stylization():
            ...
            if 'mssim' in similarity_type_weight.keys():
                sim_loss += similarity_type_weight['mssim']*pytorch_msssim.msssim(content_image, reversed_img, normalize=True) # added term for similarity loss
            ...
            if 'lpsis' in similarity_type_weight.keys():
                sim_loss += similarity_type_weight['lpsis']*lpsis_model.forward(stylized_img, reversed_img)[0][0][0][0]
